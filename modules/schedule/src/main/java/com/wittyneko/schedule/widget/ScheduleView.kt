...
class ScheduleView : FrameLayout {
    ...
    private val serviceTime get() = System.currentTimeMillis() - timeOffset //服务器时间
    private val zeroTime = LocalTime.MIDNIGHT //开始时间
    private val durationItemMin = Duration.standardMinutes(5) //item最小时间
    private val duration24 = Duration.standardHours(24)
    private val durationMin = Duration(0) // 可调整最小时间
    private val durationMax = duration24 // 可调整最大时间
    private val timePattern = "HH:mm"
    private val time24String = "24:00"

    private val layoutInterface by lazy { LayoutInflater.from(context) }
    ...
    private val topSpace = resources.idp(8)
    private val bottomSpace = resources.idp(8)
    private val itemSpace = resources.idp(45)
    private val itemMarginStart = resources.idp(56)
    private val itemMarginEnd = resources.idp(15)
    private val columnMargin = resources.idp(1)
    private val itemWidth get() = measuredWidth - itemMarginStart - itemMarginEnd + columnMargin
    private val smoothScrollSpace = resources.idp(8) //每次滑动距离
    private val timeTextOffSet = resources.idp(7) // 隐藏时间距离偏移

    private val currentTimeColor = resources.color(R.color.blue_ff4d6eff)
    private val bgTimeColor = resources.color(R.color.color_BCC1CD)

    //最小高度
    private val minHeight = 60

    private val lineViewList = arrayListOf<View>()
    private val timeViewList = arrayListOf<TextView>()
    private val itemViewList = arrayListOf<ScheduleItem>()
    var itemEditView: ScheduleItem? = null
        private set

    private val scrollView by lazy { parentAsView<NestedScrollView>() }
    ...
    private var selectedDate = LocalDate.now() //当前选中日期
    private var mAdapter: Adapter<Any>? = null

    private var itemLongClickListener = OnLongClickListener {
        //编辑Item
        editItem(it.asView())
        true
    }
    ...
    private var editClickListener = OnClickListener {
        hideEdit()
    ...
    private var cancelClickListener = OnClickListener {
        hideEdit()
        itemEditView?.also { item ->
            moveItem(item, editView.startPeriod, editView.endPeriod)
            itemEditView = null
    ...
    override fun onInterceptTouchEvent(ev: MotionEvent): Boolean {
        ...
        when (ev.action) {
            ...
            MotionEvent.ACTION_MOVE -> {
                ...
                if (abs(ev.y - mDownFocusY) > viewConfiguration.scaledTouchSlop * 0.3) {
...
//                        }
     ...
     */
    private fun showEdit(startPeriod: Period, endPeriod: Period = startPeriod.plusHours(1)) {
     ...
     * @param endPeriod Period
     */
    private fun moveItem(item: ScheduleItem, startPeriod: Period, endPeriod: Period) {
     ...
     * @param item TimeTableItem
     */
    private fun editItem(item: ScheduleItem) {
        isCheckTouch = true
        itemEditView = item
        parent.requestDisallowInterceptTouchEvent(true)
     ...
     */
    private fun createItem(startPeriod: Period, endPeriod: Period) = run {
     ...
     * @return Float
     */
    private fun timeToDistance(time: Period, offset: Int = topSpace) = run {
     ...
     * @return Period
     */
    private fun Period.round(round: Int = 15): Period {
...
//                            )
     ...
     */
    private fun ScheduleItem.updateLayoutParams() {
        ...
        if (timeToDistance(endPeriod - startPeriod, 0).toInt() < minHeight) {
     ...
     */
    private fun ScheduleEdit.updateLayoutParams() {
     ...
     */
    private fun ScheduleCurrentTime.updateLayoutParams() {
        val localTime = LocalTime(serviceTime)
        ...
        itemViewList.forEach { item ->
            ...
            val (bg, left, color) = when {
                isToady && local > end -> Triple(
                    ...
                    R.drawable.bg_schedule_item_past_left, R.color.gray_666
                )
                isToady && local in item.run { start..end } -> Triple(
                    R.drawable.bg_schedule_item_now,
                    R.drawable.bg_schedule_item_now_left2, R.color.green_ff307d4a
                )
                else -> Triple(
                    R.drawable.bg_schedule_item_future,
                    R.drawable.bg_schedule_item_future_left, R.color.green_ff307d4a
                )
            }

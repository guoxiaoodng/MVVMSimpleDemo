...
import com.data.mobile.net.constance.http.HttpPortConst
import com.data.mobile.net.converter.GsonConverter
import com.data.mobile.resouce.arouter.ARouterUtils
import com.data.mobile.resouce.arouter.constance.ARouterConstance
import com.data.mobile.resouce.bean.KeyValueInterface
import com.data.mobile.resouce.bean.MediaBean
import com.data.mobile.resouce.bean.UploadDataBean
import com.data.mobile.resouce.engine.PictureSelectionUtil
 ...
 *@date: 2021/11/10
 *@time: 14:09
 *@description：与 KeyValueMediaListOneView 的不同是 删除方式不同 这里是点击长按删除
 * 键值对  view 左边是列名称 右边是列值 eg: 名称   王XX
 * 1.kv  都要支持 设置drawable
 * 2.value 多张图片
 */
class KeyValueMediaListView : LinearLayoutCompat, View.OnClickListener {
    var keyView: AppCompatTextView? = null
    var deleteHint: AppCompatTextView? = null
    ...
    private fun initView() {
        try {
            ...
            deleteHint = view.findViewById(R.id.delete_hint)
    ...
    private fun initAttrs(attrs: AttributeSet) {
        try {
            ...
            division?.visibility = if (showDivision) VISIBLE else GONE
            showStar = ta.getInt(R.styleable.KeyValueRecyclerView_show_star, 1)
            setStarShow()
            if (isEdit) {
                deleteHint!!.visibility = VISIBLE
                keyView!!.setOnClickListener(this)
                ktDrawableRight?.let { mDr = ktDrawableRight }
                setKeyDrawableRight()
            } else {
                deleteHint!!.visibility = GONE
            }
            setKeyText(kt)
            setKeyColor(ktc)
            setKeySize(kts)
...
//                                    transformations(CircleCropTransformation())
            ...
            if (isEdit)
                fileRv!!.bindingAdapter.onLongClick(R.id.item) {
     ...
     * 这里暂时定义为 json string
     */
    inline fun <reified T : KeyValueInterface> setValue(
        data: List<T>? = null,
    ) {
        try {
            if (fileRv!!.adapter != null) {
                fileRv!!.addModels(data)
                return
            }
            when (mType) {
                KeyValueListEnum.ALL, KeyValueListEnum.IMG, KeyValueListEnum.VIDEO -> {
                    fileRv!!.divider { // 构建间距
                        ...
                        onBind {
                            val img = findView<AppCompatImageView>(R.id.img)
                            val playIcon = findView<AppCompatImageView>(R.id.play_icon)
                            val model = getModel<T>().getValue().replace("\\", "/")
                            if (model.endsWith("mp4")) {
                                img.load(model) {
                                    videoFrameMillis(1)
                                    placeholder(R.drawable.image_placeholder)
...
//                                    transformations(CircleCropTransformation())
                            ...
                            if (!getModel<T>().getKey().isNullOrEmpty()) {
                                findView<AppCompatTextView>(R.id.img_text).visibility = VISIBLE
    ...
    /**上传文件*/
    private fun uploadMediaFile(fs: MutableList<File>) {
        (context as FragmentActivity).scopeDialog {
            Post<List<UploadDataBean>>(HttpPortConst.Java.SMART_PARK_UPLOAD_URL + HttpPortConst.Java.UserCenter.SMART_PARK_UPLOAD_FILE) {
                //新增上传的时候model=workOrderAdd，处置上传时model=workOrderDispose
                converter = GsonConverter()
                addHeader("unionToken", mToken)
                param("files", fs)
    ...
    private fun setKeyText(text: String? = "") {
        if (StringUtils.isEmpty(text)) {
            keyView!!.visibility = GONE
        } else {
            keyView!!.visibility = VISIBLE
            keyView!!.text = text
        }
    }
        ...
        when (v.id) {
            R.id.key -> {
                ...
                when (mType) {
                                        ...
                                        if (!result.isNullOrEmpty()) {
                                            ...
                                            for (media in result) {
                                                if (!media.compressPath.isNullOrEmpty()) {
                                                    ...
                                                    strings.add(PictureFileUtils.getPath(context,
                    ...
                    KeyValueListEnum.RECORD -> {
                        com.hjq.toast.ToastUtils.show("去选择录音文件 请来这里继续开发")
                    }
                    KeyValueListEnum.FILE -> {
                        com.hjq.toast.ToastUtils.show("去选择其它文件 请来这里继续开发")
                    }
                }
            }
        }
